...
import { SUBJECTS, getCategoriesForSubject, getSectionsForSubject, findCategory, resolveSubject } from './categories'
import type { Question, ColorSlot, DBChild, ScreenName, AppConfig } from './types'

// ── State ─────────────────────────────────────────────────────────────

let currentUser: { id: string } | null = null
let currentChild: DBChild | null = null
let childGradeNew = 2
let selectedAvatar = '🌟'
...
let gmStreak = 0
let gmBest = 0
let gmInterval: ReturnType<typeof setInterval> | null = null
let gmTimeLeft = 0
let gmColorMap: ColorSlot[] = []
let gmCurQ: Question | null = null
let gmAnswered = false

// Calm state
let calmTimer: ReturnType<typeof setInterval> | null = null
let calmReturnScreen: ScreenName = 'quiz'
...
function $all(sel: string): NodeListOf<Element> {
...
export async function authSubmit(): Promise<void> {
  ...
  const btn     = $('auth-btn') as HTMLButtonElement
  ...
  if (!email || !pass) {
    ...
    if (!email) { emailEl.style.borderColor = '#e05050'; emailEl.focus() }
    else         { passEl.style.borderColor  = '#e05050'; passEl.focus() }
    return
  }
...
;['auth-email', 'auth-pass'].forEach(id => {
  ...
  if (el) el.addEventListener('input', () => { el.style.borderColor = ''; showAuthErr('') })
})
...
async function loadProfiles(): Promise<void> {
  ...
  ;(window as Window & { _kids?: DBChild[] })._kids = kids
...
export function selectChild(idx: number): void {
  const kids = (window as Window & { _kids?: DBChild[] })._kids ?? []
  currentChild = kids[idx] ?? null
  if (!currentChild) return

  const cfg = getConfig()
  if (currentChild.settings) updateConfig(currentChild.settings as Partial<AppConfig>)
  applySettings()
...
export function openAddChild(): void {
  $('add-child-modal').classList.add('show')
  ;($('child-name') as HTMLInputElement).value = ''
  childGradeNew = 2
...
export function closeAddChild(): void {
  $('add-child-modal').classList.remove('show')
}

export function pickAvatar(el: HTMLElement): void {
  $all('.avatar-opt').forEach(b => b.classList.remove('sel'))
  el.classList.add('sel')
  selectedAvatar = el.dataset['emoji'] ?? '🌟'
}

export function pickChildGrade(g: number, btn: HTMLElement): void {
  childGradeNew = g
  $all('#cg1,#cg2,#cg3,#cg4,#cg5').forEach(b => b.classList.remove('active'))
  btn.classList.add('active')
}

export async function saveChild(): Promise<void> {
  if (!currentUser) return
...
export function showCategory(subj: string): void {
  curSubj = subj
  ...
  const emojiMap: Record<string, string> = { math: '🔢', reading: '📖', vocab: '💬' }
  const titleMap: Record<string, string> = { math: 'Math Topics', reading: 'Reading Topics', vocab: 'Vocabulary & Words' }
...
function renderQ(): void {
  ...
  $('ans-grid').innerHTML = colorMap.map(c =>
    `<button class="ans-btn ${c.id}${isLong ? ' long-ans' : ''}" id="opt-${c.idx}" onclick="pick(${c.idx})">
  ...
  colorMap.forEach(c => {
    ...
    if (el) el.style.fontSize = fontSizeForLength(String(c.value).length)
  })
...
export function pick(idx: number): void {
  ...
  if (right) document.getElementById('opt-' + right.idx)?.classList.add('correct')
  ...
  if (ok) {
    ...
    if (curQ.timesSeen >= 2) {
      ...
      if (star) fb.innerHTML += `<div class="mastery-hint">${star}</div>`
    }
...
function checkTransitionWarn(): void {
...
function renderProgress(): void {
  ...
  ;['math', 'reading', 'vocab', 'life', 'science'].forEach(s => {
    ...
    sub += `<div class="stat-row"><span class="stat-lbl">${m.name}</span>${
      pct !== null
...
function renderBadges(): void {
  ...
  ;['math', 'reading', 'life', 'science', 'vocab'].forEach(s => {
    ...
    if (el && d && d.attempts > 0) el.textContent = Math.round(d.correct / d.attempts * 100) + '%'
  })
}
...
function applySettings(): void {
  ...
  const vs = document.getElementById('vol-slider') as HTMLInputElement | null
  if (vs) vs.value = String(Math.round(cfg.volume * 100))
...
export function toggleSetting(key: keyof Pick<AppConfig, 'voice' | 'breakOn' | 'micOn'>, btn: HTMLElement): void {
...
export function setQCount(n: number, btn: HTMLElement): void {
  updateConfig({ qCount: n })
  ;['sq5', 'sq10'].forEach(id => document.getElementById(id)?.classList.remove('active'))
  btn.classList.add('active')
}

export function setBreakStyle(style: 'big' | 'small', btn: HTMLElement): void {
...
function updateBreakButtons(): void {
  ...
  for (const prefix of ['', 'game-']) {
    ...
    if (big)   big.className   = 'break-btn-big'   + (show && style === 'big'   ? ' show' : '')
    if (small) small.className = 'break-btn-small' + (show && style === 'small' ? ' show' : '')
  }
}
...
export function toggleMute(): void {
  ...
  if (btn) btn.textContent = getConfig().voice ? '🔊' : '🔇'
  if (!getConfig().voice) cancelSpeech()
}

// ── Calm Break ────────────────────────────────────────────────────────

export function startCalm(): void {
  calmReturnScreen = ($q('.screen.active') as HTMLElement | null)?.id as ScreenName ?? 'quiz'
  ...
  if (lbl)  lbl.textContent = cfg.calmDur + ' seconds'
  ...
  calmTimer = setInterval(() => {
    ...
    if (lbl)  lbl.textContent = left > 0 ? left + ' seconds' : 'Ready when you are 🌟'
    if (left <= 0 && calmTimer) clearInterval(calmTimer)
  }, 1000)
}

export function endCalm(): void {
  if (calmTimer) clearInterval(calmTimer)
  stopCalmAudio()
...
function micStateUI(state: 'idle' | 'listening' | 'heard', extra = ''): void {
...
export function handleMicToggle(): void {
  if (!getConfig().micOn) return
  toggleMic()
}

// ── Game Mode (stub — full impl carries over from original) ───────────

export function gsPickSubj(subj: string, el: HTMLElement): void {
  gmSubj = subj
  $all('.gs-subj-card').forEach(b => b.classList.remove('sel'))
  el.classList.add('sel')
}

export function gsPickTime(t: number, btn: HTMLElement): void {
  gmTime = t
...
G.startCategory = (s: string, c: string) => void startCategory(s, c)
G.pick          = pick
G.newRound      = newRound
...
setupMic({
  ...
  onInterim: (text) => {
    if (text.length > 0) micStateUI('listening', `"${text}"`)
  },
