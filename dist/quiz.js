...
export function buildColorMap(question) {
...
export function masteryClass(level) {
    return ['', 'struggling', 'practicing', 'mastered'][level] ?? '';
}
export function fontSizeForLength(len) {
    if (len <= 2)
        return 'clamp(4rem,18vw,9rem)';
    if (len <= 4)
        return 'clamp(2.8rem,12vw,6rem)';
    if (len <= 6)
        return 'clamp(2rem,8vw,4rem)';
    if (len <= 10)
...
function mkQ(text, answer, dist, eq) {
...
export function mathBank(grade) {
...
export function readingBank(grade) {
    const base = [
...
export function getFallbackBank(subj, grade) {
    switch (subj) {
